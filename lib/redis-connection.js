...
if (!process.env.REDIS_URL) {
	...
	function getStreamFromURL(url, ttl) {

		return client.existsAsync(url)
		.then(exists => {
			if (exists) {
				...
				return new Promise((resolve, reject) => {
					...
					req.on('response', function (res) {
						...
						if (ttl) {
							stream.on('end', () => client.expireAsync(url, ttl));
							resolve(stream.pipe(client.writeThrough(url)));
						} else {
							resolve({
								fresh: true,
								stream: stream
