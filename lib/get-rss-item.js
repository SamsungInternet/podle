...
module.exports = function getRSSItem(url, forceFresh) {
	...
	return redisClient.getAsync(redisHashKey)
	.then(hash => new Promise(function (resolve, reject) {
		const output = [];
		let meta;
		...
		getStreamFromURL(url, 1800, forceFresh).then(({stream}) => {
		...
		feedparser.on('finish', function () {
			...
			// if meta is undefined then define it
			meta = meta || {};
